...
- TDD 또는 Test Pattern Guide gate 변경

## 3개 시나리오

### Case 1. 신규 API 엔드포인트 추가
요청: "새 사용자 조회 API 엔드포인트를 추가해줘."
기대 흐름: request-intake-agent → feature-architect-agent(Test Pattern Guide 확인 + tasklist 생성) → 테스트 책임자 → tasklist verification/red verifier → implementer(tasklist 실행) → green verifier → refactor/quality review → doc-updater
...
□ tasklist Source에 `.agent-team/test_pattern_guide.md`와 적용한 테스트 패턴이 포함되는가
...
□ 기존 프로젝트 테스트 관례와 `.agent-team/test_pattern_guide.md`를 먼저 확인하는가
□ 실패 테스트 작성 후 Red Verification이 구현보다 먼저 실행되는가
□ Red 실패 원인이 기능 미구현인지, 실패 테스트가 Test Pattern Guide 패턴을 따르는지 확인하는가
